...
- Set `MERLOC_BROKER_HOST` environment variable to the
...
- Set `MERLOC_BROKER_HOST` environment variable (likewise you did for GateKeeper above) to the
  - value of the `merloc-broker.merlocbrokerwsapistageurl` property which you get from AWS CDK broker setup output as shown in the example below:
    ![Broker URL From AWS CDK Output](./assets/broker-url-from-aws-cdk-output.png)
...
- `MERLOC_BROKER_HOST`: This configuration is **MANDATORY**.
...
- `MERLOC_BROKER_CONNECTION_NAME`: This configuration is **OPTIONAL**.
  By default, the name of the connection to the broker is the name of the AWS Lambda function.
